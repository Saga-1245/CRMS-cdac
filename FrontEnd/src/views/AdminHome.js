...
export default function AdminHome() {
    ...
    function getAllUsers(e) {
        ...
        axios.get(`${url}/getallcustomers`).then((response) => {
    ...
    function deleteRequest(userid) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                axios.delete(`${url}/deleteuser/?id=${userid}`).then((response) => {
                    Swal.showLoading();
    ...
    return (
        <>
            ...
            <div className="adminhomebg ">
                ...
                <h1 className="text-light" style={{fontWeight:"bolder",fontStyle:"italic"}}> Welcome Admin</h1>
                <button className='btn btn-info' onClick={getAllUsers}> Get All Users</button>
                <button className='btn btn-danger ' onClick={() => setuserflag(false)}> Clear Screen</button>
...

                                        {
                                            userlist.map((user) => (
                                                <tr key={user.user_id}>
                                                    ...
                                                    <td>{user.state}</td>
                                                    <td>{user.city}</td>
                                                    <td>{user.isTransporter ? "Transporter" : "Customer"} </td>
